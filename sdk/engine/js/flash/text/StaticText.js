...
(function () {
  ...
  d._updateCanvas = function () {
    ...
    var textureWidth = flash.getTextureSize(
      this._bounds.width * this._scaleCorrection,
    );
    var textureHeight = flash.getTextureSize(
      this._bounds.height * this._scaleCorrection,
    );

    if (
      this._bitmapData &&
      this._bitmapData.width == textureWidth &&
      this._bitmapData.height == textureHeight
    ) {
      this._bitmapData.fillRect(
        0,
        0,
    ...
    for (var i = 0; i < this._textRecords.length; i++) {
      ...
      for (var j = 0; j < text.length; j++) {
        context.fillText(text[j], x - xShift, y - yShift);
