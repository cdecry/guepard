...
(function () {
  ...
  d.load = function (
    ...
    if (!this._contentLoaderInfo._url) {
      this._contentLoaderInfo.dispatchEvent(
        new flash.events.IOErrorEvent(flash.events.IOErrorEvent.IO_ERROR),
      );
      return;
    }
    ...
    if (extension == "jpg" || extension == "png" || extension == "gif") {
      ...
      this._urlLoader.addEventListener(
        flash.events.Event.COMPLETE,
        flash.bindFunction(this, this._complete),
      );
  ...
  d._progress = function (value) {
    this._contentLoaderInfo._bytesLoaded = Math.round(
      value * this._contentLoaderInfo.get_bytesTotal(),
    );
  ...
  d._complete = function (e) {
    ...
    this._urlLoader.removeEventListener(
      flash.events.Event.COMPLETE,
      flash.bindFunction(this, this._complete),
    );
  ...
  d._parseSWF = function (data) {
    ...
    var domain = (flash.system.ApplicationDomain._currentDomain =
  ...
  d._loadSWFImage = function () {
    this._progress(
      0.1 +
        0.9 *
          flash.system.ApplicationDomain.get_currentDomain()._getloadInagesProgress(),
    );

    this._imagePath =
      flash.system.ApplicationDomain.get_currentDomain()._getImageForLoad();
    ...
    if (this._imagePath) {
      ...
      loader
        .get_contentLoaderInfo()
        .addEventListener(
          flash.events.Event.COMPLETE,
          flash.bindFunction(this, this._loadSWFImageComplete),
        );
      loader.load(request);
    } else {
      //return;
  ...
  d._loadSWFImageComplete = function (e) {
    ...
    this._contentLoaderInfo._applicationDomain._setImage(
      this._imagePath,
      bitmap,
    );
  ...
  d._loadSWFFont = function () {
    this._font =
      flash.system.ApplicationDomain.get_currentDomain()._getFontForLoad();

    if (this._font) {
      var loader = new flash.text.CSSFontLoader(/*path,*/ this._font);
      loader.addEventListener(
        flash.events.Event.COMPLETE,
        flash.bindFunction(this, this._loadSWFFontComplete),
      );
    } else {
      this.addEventListener(
        flash.events.Event.ENTER_FRAME,
        flash.bindFunction(this, this._loadSWFContentComplete),
      );
    }
  };
  ...
  d._addBitmapObject = function () {
    var bitmapData = new flash.display.BitmapData(
      this._imageObj.width,
      this._imageObj.height,
      true,
      0x00000000,
    );
    ...
    try {
      ...
      this._contentLoaderInfo.dispatchEvent(
        new flash.events.SecurityErrorEvent(
  ...
  s.__init__ = function () {
    ...
    /*super*/
    this.prototype.DisplayObjectContainer_addChildAt =
      this.__base__.prototype.addChildAt;
  };
