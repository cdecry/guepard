...
(function () {
  ...
  d.get_maxScrollV = function () /*int*/
  {
    ...
    var lines = Math.ceil(
      lineHeight > 0 ? this._bounds.height / lineHeight : this.get_numLines(),
    );
  ...
  d.TextField = function () {
    ...
    this._defaultTextFormat = new flash.text.TextFormat(
      "Times New Roman",
      12,
      0x000000,
      false,
  ...
  {
    return this._linesMetrics && this._linesMetrics.length
      ? this._linesMetrics[0].height
      : 0;
  };
  ...
  d._upateInput = function (matrix /*Matrix*/ /*void*/) {
    ...
    if (!this._input) {
      ...
      this._input.style.direction =
        this._defaultTextFormat.get_align() == "right" ? "rtl" : "ltr";
      ...
      this._input.onfocus = function (e) {
        that.dispatchEvent(
          new flash.events.FocusEvent(flash.events.FocusEvent.FOCUS_IN),
        );
      };

      this._input.onblur = function (e) {
        that.dispatchEvent(
          new flash.events.FocusEvent(flash.events.FocusEvent.FOCUS_OUT),
        );
    ...
    if (this.get_visible()) {
      ...
      this._setStyleProperty(
        ...
        "left",
        stageMatrix.tx +
          stageMatrix.a * matrix.tx +
          this._bounds.x * stageMatrix.a * matrix.a +
          "px",
      );
      this._setStyleProperty(
        this._input,
        "top",
        stageMatrix.ty +
          stageMatrix.d * matrix.ty +
          this._bounds.y * stageMatrix.d * matrix.d +
          "px",
      );
      this._setStyleProperty(
        this._input,
        "width",
        this._bounds.width * stageMatrix.a * matrix.a + "px",
      );
      this._setStyleProperty(
        this._input,
        "fontSize",
        textFormat.get_size() * stageMatrix.a * matrix.a + "px",
      );
      ...
      this._setStyleProperty(
        this._input,
        "color",
        flash.numberToColor(textFormat.get_color()),
      );
    ...
    if (!this._dataUpdated) {
      ...
      for (var i = 0; i < formatedtext.length; i++) {
        if (typeof formatedtext[i] == "string") {
          ...
          nextLetter =
            typeof formatedtext[i + 1] == "string"
              ? formatedtext[i + 1]
              : formatedtext[i + 2];
          ...
          if (currentDefineFont) {
            advance = currentDefineFont.getCharsAdvance(
              letter,
              nextLetter,
              fontName,
              size,
          ...
          if (!currentDefineFont) {
            var findedFont =
              flash.system.ApplicationDomain.get_currentDomain()._getFont(
                currentFormat._font,
              );
      ...
      if (this._autoSize == flash.text.TextFieldAutoSize.NONE) {
        ...
        ch =
          this._bounds.height > lineHeight ? this._bounds.height : lineHeight;
      } else {
        cw = textBounds.width;
  ...
  d._updateCanvas = function () {
    if (!this._canvasUpdated) {
      ...
      if (!this._bitmapData) {
        this._bitmapData = new flash.display.BitmapData(
          textureWidth,
          textureHeight,
          true,
          0,
      ...
      if (lineletters[0].format._align == "justify") {
        if (lineMetrics != linesMetrics[linesMetrics.length - 1]) {
          if (this._bounds.width * 0.8 < lineMetricswidth) {
            space =
              (this._bounds.width - lineMetricswidth) / lineletters.length;
          }
        }
      }
    } else {
    ...
    while (linesLength > 0) {
      ...
      while (lineLength > 0) {
        ...
        while (wordLength > 0) {
          ...
          lineHeight =
            word[wordLength].ascent +
            word[wordLength].leading +
            word[wordLength].descent;
    ...
    for (var i = 1; i < textLength; i++) {
      ...
      if (currentFormatLength == formatLength) {
        while (currentFormatLength) {
          if (
            currentFormat[currentFormatLength] != format[currentFormatLength]
          ) {
            newformat = true;
            break;
          }
  ...
  d._getMixedFormat = function (format, formatLength) {
    ...
    for (var i = 0; i < formatLength; i++) {
      ...
      if (format[i]._definefont != null)
        mixedFormat._definefont = format[i]._definefont;
      ...
      if (format[i]._underline != null)
        mixedFormat._underline = format[i]._underline;
      ...
      if (format[i]._leftMargin != null)
        mixedFormat._leftMargin = format[i]._leftMargin;
      if (format[i]._rightMargin != null)
        mixedFormat._rightMargin = format[i]._rightMargin;
  ...
  d.appendText = function (newText) {
    ...
    for (var i in newText) {
      this._formatedtext.push({
        character: newText[i],
        textformats: [this._defaultTextFormat],
      });
    }
  ...
  d.setTextFormat = function (format, beginIndex, endIndex) {
    ...
    if (
      beginIndex > this._formatedtext.length ||
      beginIndex < -1 ||
      endIndex < -1 ||
  ...
  /*public*/
  d.getTextFormat = function (
    beginIndex /*int*/,
    endIndex /*int*/ /*TextFormat*/,
  ) {
  ...
  /*public*/
  d.insertXMLText = function (
    beginIndex /*int*/,
    endIndex /*int*/,
    richText /*String*/,
    pasting /*Boolean*/ /*void*/,
  ) {};
