(function () {
  ...
  d.createDisplayObject = function () {
    ...
    if (this.id) {
      var symbolClass =
        flash.system.ApplicationDomain.get_currentDomain()._getLinkage(this.id);
  ...
  d.initScenes = function (movieClip, defineScene) {
    ...
    for (var i = 0; i < defineScene.scenes.length; i++) {
      ...
      for (var j = 0; j < defineScene.labels.length; j++) {
        ...
        if (
          sceneObject.offset <= labelObject.frame &&
          labelObject.frame < sceneObject.offset + numFrames
        ) {
      ...
      var scene = new flash.display.Scene(sceneObject.name, labels, numFrames);
