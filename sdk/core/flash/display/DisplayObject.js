...
(function () {
  ...
  d._getGraphicsBounds = function (
    ...
    if (localBounds && !localBounds.isEmpty()) {
      ...
      if (targetCoordinateSpace) {
        var targetMatrix = targetCoordinateSpace
          .get_transform()
      ...
      this._expandLimits(
        limits,
        matrix,
        new flash.geom.Point(localBounds.x, localBounds.y + localBounds.height),
      );
      this._expandLimits(
        limits,
        matrix,
        new flash.geom.Point(localBounds.x + localBounds.width, localBounds.y),
      );
  ...
  d.__updateTransform__ = function () {
    ...
    if (parent) {
      transform._concatenatedMatrix.concat(
        parent._transform._concatenatedMatrix,
      );
      transform._concatenatedColorTransform.concat(
        parent._transform._concatenatedColorTransform,
      );

      if (parent._concatenatedFilters.length) {
        this._concatenatedFilters = this._concatenatedFilters.concat(
          parent._concatenatedFilters,
        );
      }
    }

    transform._invertedConcatenatedMatrix.copyFrom(
      transform._concatenatedMatrix,
    );
  ...
  d.addEventListener = function (
    ...
    this.EventDispatcher_addEventListener(
      type,
      listener,
      useCapture,
      priority,
  ...
  d._dispatchInteractiveEvent = function (data, x, y, target, type) {
    ...
    if (type == flash.events.MouseEvent.MOUSE_DOWN) {
      ...
      flash.display.DisplayObject.__pressedObjects.indexOf(this) != -1
    ) {
      this._dispatchInteractiveEvent(
        data,
        x,
        y,
        target,
  ...
  s.__enterFrame__ = function () {
    ...
    for (var i = 0; i < objects.length; i++) {
      ...
      object.dispatchEvent(
        new flash.events.Event(flash.events.Event.ENTER_FRAME),
      );
    }
  };
  ...
  s.__exitFrame__ = function () {
    ...
    for (var i = 0; i < objects.length; i++) {
      ...
      object.dispatchEvent(
        new flash.events.Event(flash.events.Event.EXIT_FRAME),
      );
    }
  };
  ...
  s.__init__ = function () {
    ...
    /*super*/
    /*public*/
    this.prototype.EventDispatcher_addEventListener =
      this.__base__.prototype.addEventListener;
    /*super*/
    /*public*/
    this.prototype.EventDispatcher_removeEventListener =
      this.__base__.prototype.removeEventListener;
