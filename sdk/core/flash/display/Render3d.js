(function () {
  ...
  d.Render3d = function (canvasId, width, height) {
    ...
    this._colorTransformMatrix = [1, 1, 1, 1, 0, 0, 0, 0, 0];
    ...
    this._u_imageLocation = gl.getUniformLocation(
      this._shaderProgram,
      "u_image",
    );
    ...
    this._colorTransform = gl.getUniformLocation(
      this._shaderProgram,
      "u_colortransform",
    );
    gl.uniformMatrix3fv(
    ...
    this._matrixLocation = gl.getUniformLocation(
      this._shaderProgram,
      "u_matrix",
    );
    ...
    this._projectionMatrix = gl.getUniformLocation(
      this._shaderProgram,
      "pMatrix",
    );
    gl.uniformMatrix4fv(this._projectionMatrix, false, [
      2.0 / this._width,
      0,
      0,
      0,
    ...
    this._identy_matrix = [1, 0, 0, 0, 1, 0, 0, 0, 1];

    this._matrix = [1, 0, 0, 0, 1, 0, 0, 0, 1];

    this._usermatrix = [1, 0, 0, 0, 1, 0, 0, 0, 1];
  ...
  d._addToBatch = function (textureInfo) {
    ...
    this._batch.texCoordBuffer.push(tx0, ty0, tx1, ty0, tx0, ty1, tx1, ty1);

    var batchindices_offset = this._batchindices_offset;

    this._batch.vertexIndexBuffer.push(
      0 + batchindices_offset,
      1 + batchindices_offset,
      3 + batchindices_offset,
      0 + batchindices_offset,
      3 + batchindices_offset,
      2 + batchindices_offset,
    );
  ...
  d._drawText = function (child) {
    ...
    gl.drawElements(
      gl.TRIANGLES,
      textfield.fontVertexIndexBuffer.numItems,
      gl.UNSIGNED_SHORT,
      0,
    );
  ...
  d._initTextBuffers = function (textfield) {
    ...
    var fontVertexIndexBuffer =
      textfield.fontVertexIndexBuffer == null
        ? gl.createBuffer()
        : textfield.fontVertexIndexBuffer;
    textfield.fontVertexIndexBuffer = fontVertexIndexBuffer;

    var fontVertexPositionBuffer =
      textfield.fontVertexPositionBuffer == null
        ? gl.createBuffer()
        : textfield.fontVertexPositionBuffer;
    textfield.fontVertexPositionBuffer = fontVertexPositionBuffer;

    var textFieldTexCoordBuffer =
      textfield.textFieldTexCoordBuffer == null
        ? gl.createBuffer()
        : textfield.textFieldTexCoordBuffer;
  ...
  d._renderBatch = function () {
    ...
    gl.bufferData(
      gl.ARRAY_BUFFER,
      new Float32Array(batch.texCoordBuffer),
      gl.STATIC_DRAW,
    );
    ...
    gl.bufferData(
      gl.ARRAY_BUFFER,
      new Float32Array(batch.vertexPositionBuffer),
      gl.STATIC_DRAW,
    );
    ...
    gl.bufferData(
      gl.ELEMENT_ARRAY_BUFFER,
      new Uint16Array(batch.vertexIndexBuffer),
      gl.STATIC_DRAW,
    );
    ...
    gl.drawElements(
      gl.TRIANGLES,
      batchVertexIndexBuffer.numItems,
      gl.UNSIGNED_SHORT,
      0,
    );
  ...
  d.setSize = function (w, h) {
    ...
    this._gl.uniformMatrix4fv(this._projectionMatrix, false, [
      2.0 / w,
      0,
      0,
      0,
