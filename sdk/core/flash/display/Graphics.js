...
(function () {
  ...
  d.drawEllipse = function (
    ...
    if (this._path != null) {
      ...
      this._path.commands.push(
        flash.display.GraphicsPathCommand.CUBIC_CURVE_TO,
      );
  ...
  d.drawRoundRect = function (
    ...
    this._path.commands.push(flash.display.GraphicsPathCommand.LINE_TO);
    this._path.data.push(xm + dx, y);

    this._path.commands.push(flash.display.GraphicsPathCommand.CUBIC_CURVE_TO);
    this._path.data.push(xm + ox + dx, y, xe + dx, ym - oy, xe + dx, ym);

    this._path.commands.push(flash.display.GraphicsPathCommand.LINE_TO);
    this._path.data.push(xe + dx, ym + dy);
    ...
    this._path.commands.push(flash.display.GraphicsPathCommand.LINE_TO);
    this._path.data.push(xm, ye + dy);

    this._path.commands.push(flash.display.GraphicsPathCommand.CUBIC_CURVE_TO);
    this._path.data.push(xm - ox, ye + dy, x, ym + oy + dy, x, ym + dy);
  ...
  /*public*/
  d.drawRoundRectComplex = function (
    x /*Number*/,
    y /*Number*/,
    width /*Number*/,
    height /*Number*/,
  ...
  d.lineStyle = function (
    ...
    var lineStyle = new flash.display.GraphicsStroke(
      thickness,
      pixelHinting,
      scaleMode,
      caps,
  ...
  d._getBoundingBox = function () {
    if (this._data.length) {
      ...
      if (
        ...
        if (width != 0 || height != 0) {
          var bounds = new flash.geom.Rectangle(xMin, yMin, width, height);
  ...
  d._checkTouch = function (x, y, displayObject) {
    ...
    if (bounds) {
      ...
      this._checkTouch_point = displayObject.globalToLocal(
        this._checkTouch_point,
      );
      ...
      if (bounds.containsPoint(this._checkTouch_point)) {
        if (this._textureInfo) {
          ...
          var bitmapX = Math.floor(
            map.x +
              ((this._checkTouch_point.x - bounds.x) / bounds.width) *
                map.width,
          );
          var bitmapY = Math.floor(
            map.y +
              ((this._checkTouch_point.y - bounds.y) / bounds.height) *
                map.height,
          );
  ...
  d._getColorTransfomedData = function (transform) {
    if (!this._cTransformedvirtualcanvas) {
      ...
      this._cTransformedcontext2d =
        this._cTransformedvirtualcanvas.getContext("2d");
      ...
      this._colorTransfomedImgData =
        this._cTransformedcontext2d.createImageData(
          _canvasRect.width,
          _canvasRect.height,
        );
    }
    ...
    if (
      (this._colorTransfomedData[0] == transform[0]) &
      ...
      if (
        this._colorTransfomedImgData.width != this._canvasRect.width ||
        this._colorTransfomedImgData.height != this._canvasRect.height
      ) {
        ...
        this._colorTransfomedImgData =
          this._cTransformedcontext2d.createImageData(
            ...
            this._canvasRect.height,
          );
      } else {
        this._cTransformedcontext2d.clearRect(
          0,
          0,
          this._canvasRect.width,
          this._canvasRect.height,
      ...
      for (var i = 0; i < _colorTransfomedImgData.data.length; i += 4) {
        colorTransfomedImgDataData[i] =
          colorTransfomedImgDataData[i] * this._colorTransfomedData[0] +
          this._colorTransfomedData[3];
        colorTransfomedImgDataData[i + 1] =
          colorTransfomedImgDataData[i + 1] * this._colorTransfomedData[1] +
          this._colorTransfomedData[4];
        colorTransfomedImgDataData[i + 2] =
          colorTransfomedImgDataData[i + 2] * this._colorTransfomedData[2] +
          this._colorTransfomedData[5];
      }

      this._cTransformedcontext2d.putImageData(
        this._colorTransfomedImgData,
        0,
        0,
      );
