...
amxmlc \
  -source-path src \
  -library-path+=lib/blooddyCrypto/blooddy_crypto.swc \
  -library-path+=lib/design/design.swc \
  -library-path+=lib/zip/fzip.swc \
  -output converter.swf \
  src/com/guepard/app/Converter.as
```
